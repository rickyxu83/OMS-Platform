...
async function run() {
  ...
  const buffer = await workbookBuffer([
    ...
    ['服务器四', 'SN-A004', 'IT Basic Warranty', '2020-03-14', '2023-03-13'],
    ['服务器四', 'SN-A004', 'Hi-Care', '2020-04-26', '2023-04-25'],
  ...
  const devices = [
    ...
    { id: 4, serialNo: 'SN-A004', customerName: '丁客户', model: 'OceanStor', maintenanceType: 'original_manufacturer', maintenanceStart: null, maintenanceEnd: null },
  ]
  ...
  assert.strictEqual(preview.summary.updatable, 3)
  assert.strictEqual(preview.summary.conflicts, 1)
  assert.strictEqual(preview.summary.invalid, 0)
  assert.strictEqual(preview.summary.ignored, 2)
  ...
  assert.strictEqual(preview.items.filter((item) => item.serialNo === 'SN-A003' && item.status === 'updatable').length, 1)
  assert.strictEqual(preview.items.filter((item) => item.serialNo === 'SN-A003' && item.status === 'superseded').length, 1)
  const selectedHuaweiService = preview.items.find((item) => item.serialNo === 'SN-A004' && item.status === 'updatable')
  assert.strictEqual(selectedHuaweiService.maintenanceStart, '2020-04-26')
  assert.strictEqual(selectedHuaweiService.maintenanceEnd, '2023-04-25')
  assert.strictEqual(preview.items.filter((item) => item.serialNo === 'SN-A004' && item.status === 'superseded').length, 1)
