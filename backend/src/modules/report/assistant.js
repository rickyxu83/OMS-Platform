...
const SYSTEM_RULES = [
  ...
  '15. 明细模式：用户要「明细/清单/逐条记录/月报/工作记录导出」时用 detail 填明细列 key（数据目录里标了 detailColumns 的数据集才支持）。月报默认列：order_no/engineer/date/weekday/work_nature/customer/main_content/progress（不含 remark/source/product/work_content，用户要求时可加）；用户对话中增删列 → 调整 detail 数组；导出月报类多 sheet 文件时 sheetBy 填分组列（如 engineer）。明细模式不需要 metrics/groupBy/compare，timeRange 照常作用于工作时间',
  '',
  'final 回复规则：',
