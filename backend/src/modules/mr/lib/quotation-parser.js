...
function classifyWorkbook(sheets, fileName = '') {
  ...
  if (sheets.some((sheet) => sheet.customer && !/(dunyang|敦阳|敦陽|stark|敦阳上海|敦陽上海|dunhu|敦沪)/i.test(sheet.customer))) return 'sales_quote'
  if (/(购货单位|購貨單位|购买单位|客户名称[：:]敦阳|客戶名稱[：:]敦陽|客户名称[：:]敦沪|to:\s*stark|to：stark|to:\s*敦阳|to：敦陽|to:\s*敦沪|to：敦沪)/i.test(normalized)) return 'purchase_quote'
  if (/(stark|dunyang|敦阳|敦陽|dunhu|敦沪)/i.test(fileName)) return 'purchase_quote'
  return 'unknown'
}
