...
const COMPANIES = [
  ...
  { value: 'dunhu', label: '上海敦沪信息科技有限公司', shortLabel: '敦沪', enName: 'Shanghai Stark Technology Inc.', pricingModes: [3] },
]
...
// 供应商为敦阳/敦沪（含繁体/英文写法）表示内部承担、无外部下单对象（2026-09-08 佬裁决；2026-09-10 spec 011 加敦沪）。
// 用途（spec 013 起）：①采购提交时内部承担品项不强制采购单号（但公司料号仍必填）；②报价导入的自己公司识别同口径。
// 注意：签核通过不再据此跳过采购——所有 MR 一律流转采购填料号。
...
function validateSubmission(order, items) {
  ...
  for (const [field, label] of [['purchaserMail', '采购联系人'], ['recipientMail', '收货人'], ['invoiceRecipientMail', '发票收件人']]) {
