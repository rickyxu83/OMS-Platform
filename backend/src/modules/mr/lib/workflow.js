...
async function ensureWorkflowTables() {
  ...
  // 任务类型：purchase=采购填写（料号/采购单号）；contract_no=合同编号补填（有合同但签核时暂无编号）
...
async function createPurchaseTask(connection, order, purchaser, initiatorUserId, event = 'purchase_task') {
  ...
  await connection.execute(
    ...
    {
      mrId: order.id,
      title: `${order.customerName || '未选客户'} · 采购填写（料号/单号）`.slice(0, 255),
...
const MERGED_TASKS_SUBQUERY = `
       ...
       SELECT t.id, CASE WHEN t.task_type = 'contract_no' THEN 'mr_contract_no' ELSE 'mr_purchase' END AS business_type, t.mr_id AS business_id, NULL AS approval_id,
              ...
              o.status AS business_status, o.purchase_status AS business_purchase_status, o.void_request_status AS business_void_status, CASE WHEN t.task_type = 'contract_no' THEN '合同编号补填' ELSE '采购填写（料号/单号）' END AS current_step_label, o.customer_name, o.ctrl_no
