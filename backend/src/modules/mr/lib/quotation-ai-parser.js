...
const USER_PROMPT = [
  ...
  '3. name 保留品名原文（可修正明显的 OCR 错字）；description 可包含规格型号；partNo 为型号/料号：优先填写文件中的料号、序列号或型号编码（例如 952145001204/952145001351 这类数字编码，多个编码用 / 连接），不要填 MISC、FAS2750、维修、维保等短词或品名，没有可靠型号编码则填空字符串',
