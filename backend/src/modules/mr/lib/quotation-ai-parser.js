...
const USER_PROMPT = [
  ...
  '3. name 必须包含完整品名：若文件中“产品名称”与“型号及规格”分列，name 应组合为“产品名称 + 规格”（例如 “6类非屏蔽跳线 1米”、“LC-LC OM4光纤跳线 15米”），不得只填规格（如单独的 “1米”“15米”）；description 同样应包含产品名称与规格；partNo 为型号/料号：优先填写文件中的料号、序列号或型号编码（多个编码用 / 连接）；如文件中出现序列号（尤其是 NetApp 设备，每台通常有一组两个序列号，多份文件中前后顺序可能不同），必须把所有序列号全部写入 partNo，不要遗漏；不要填 MISC、FAS2750、维修、维保等短词或品名，没有可靠型号编码则填空字符串',
