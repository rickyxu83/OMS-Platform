...
## 分期（佬裁决 2026-09-16：不分期，全部一次做好）

- 对话生成 → 表格预览 + 图表 + 文字摘要 + Excel/PDF 导出 + 模板收藏：一次交付。
- 数据集一次覆盖全部六类：工单、工时、考勤、巡检计划、设备、订购申请（MR）。
- 定时生成 + 邮件推送（周刊每周一、月刊每月 1 日，复用 `services/scheduler.js` 与邮件设置）同步交付。
- 图表库选用 recharts（懒加载，仅智能报表页加载）。
...
1. ~~"主管及以上"口径~~：已裁决——含助理主管、业务主管（见「使用对象」表）。
2. ~~P1 数据集范围~~：已裁决——六类数据集一次全做。
3. ~~图表放 P2~~：已裁决——图表与 PDF、订阅推送一次交付（recharts）。
