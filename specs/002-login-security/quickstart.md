...
### S5b：陌生设备登录邮件提醒（FR-011）

> 前置：RN 管理端「系统设置 → 邮件」中 `mail.enabled` 打开（当前关闭，考勤等通知会一并激活，验收后可关回）。

1. 用一个从未登过的浏览器（或清 Cookie）密码登录 → 落 `oms_device_id` Cookie → 账号邮箱收到「新设备登录提醒」，含时间/登录方式/设备/IP 归属地。
2. 同一浏览器再次退出登录 → 不再发邮件。
3. 通行密钥登录（新浏览器）→ 同样收到提醒，登录方式标注「通行密钥（生物识别）」。
4. 关闭邮件服务时登录 → 登录本身不受影响（静默降级）。
