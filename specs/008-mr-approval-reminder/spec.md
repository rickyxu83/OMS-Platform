...
- 催办入口三处，同一接口同一节流：
  1. MR 详情页操作栏「催办」按钮（可见口径：创建人/业务负责人/本单助理 + `in_review`）
  2. 待办中心「我发起的」视图：签核中的 MR 行内「催办」按钮（发起人视角）
  3. MR 列表页：签核中状态行内铃铛图标（按行 `permissions.canRemind` 控制显隐）
  - 24h 内置灰并提示上次催办时间；列表/待办接口均返回 `lastRemindedAt`
