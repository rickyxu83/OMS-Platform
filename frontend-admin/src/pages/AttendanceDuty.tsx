...
export function AttendanceDuty({ embedded = false }: { embedded?: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {tab === "setup" ? <div className="space-y-5">
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><span className="flex size-5 items-center justify-center rounded bg-rose-100 text-[11px] font-bold text-rose-700">假</span>法定节假日值班</CardTitle><CardDescription>只需为实际安排值班的节假日选择工程师；与 7×24 独立设置，同一工程师同一天重叠时，月度提交前必须选择保留一项。</CardDescription></CardHeader><CardContent className="space-y-5">{holidays.length ? holidays.map((holiday, index) => <div key={holiday.date} className="rounded-lg border p-4 transition hover:border-rose-200"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><span className="font-medium">{holiday.name}</span><span className="rounded-full bg-rose-50 px-2.5 py-0.5 text-xs tabular-nums text-rose-700 ring-1 ring-inset ring-rose-200">{holiday.date} {dutyWeekday(holiday.date)}</span></div><span className={`text-xs ${holiday.employeeIds.length ? "font-medium text-rose-700" : "text-muted-foreground"}`}>{holiday.employeeIds.length ? `已选 ${holiday.employeeIds.length} 人` : "不安排值班"}</span></div><EngineerPicker compact engineers={engineers} selected={holiday.employeeIds} onToggle={(id) => setHolidays(holidays.map((item, itemIndex) => itemIndex === index ? { ...item, employeeIds: toggle(item.employeeIds, id) } : item))} /></div>) : <div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">该年度尚未维护法定节假日，请先在考勤设置中维护。</div>}</CardContent></Card>
