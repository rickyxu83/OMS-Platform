...
export function Login() {
  ...
  const i18n = {
    "zh-CN": {
      ...
      passkeyTip: "通行密钥 = 免密码登录，用设备自带的验证（指纹 / 人脸 / PIN 码）代替密码。① 登记（每台设备一次）：密码登录后进入「我的设置 → 通行密钥 → 登记本设备」，按提示完成验证——手机、Mac/Windows 笔记本、台式机都可以登记。② 登录：输入邮箱点「继续」，已登记过的设备会直接弹出指纹/人脸/PIN 验证，通过即登录。③ 电脑没登记过：屏幕会出现二维码，用已登记的手机扫码、刷脸即可（需打开手机和电脑的蓝牙）。支持方式：iPhone/iPad 的 Face ID、Mac 的 Touch ID 或开机密码、Windows 的 Hello（人脸/指纹/PIN 码）、安卓的指纹或人脸。",
    ...
    "zh-TW": {
      ...
      passkeyTip: "通行密鑰 = 免密碼登錄，用裝置自帶的驗證（指紋 / 人臉 / PIN 碼）代替密碼。① 登記（每台裝置一次）：密碼登錄後進入「我的設定 → 通行密鑰 → 登記本裝置」，按提示完成驗證——手機、Mac/Windows 筆電、桌上型電腦都可以登記。② 登錄：輸入信箱點「繼續」，已登記過的裝置會直接彈出指紋/人臉/PIN 驗證，通過即登錄。③ 電腦沒登記過：螢幕會出現 QR Code，用已登記的手機掃碼、刷臉即可（需開啟手機和電腦的藍牙）。支援方式：iPhone/iPad 的 Face ID、Mac 的 Touch ID 或開機密碼、Windows 的 Hello（人臉/指紋/PIN 碼）、安卓的指紋或人臉。",
