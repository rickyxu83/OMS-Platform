 ...
 * 功能开关（暗启动）。
 *
 * MR 模块（订购申请 + 统一待办）与手机签名（user-signature）已于 2026-08-27 点亮：
 * 菜单/路由/轮询常驻，不再受开关控制（生产 compose 的 FEATURE_MODULES_DISABLED
 * 需同步移除 mr,approval-tasks,user-signature）。
 *
 * 考勤系统仍为半成品（待 QA 与灰度），随整体发布合入 main 后，
 ...
 * - 生产（未注入 test）：隐藏
 *
 * 点亮路径：考勤 QA+灰度完成后，删除本开关及全部引用（菜单/路由恢复常驻），
