...
function ItemTable({ items, emptyText, formal }: { items: MrItem[]; emptyText: string; formal: boolean }) {
  const definitions = [
    ...
    { key: 'installBy', label: '装机方', weight: 5, align: 'left', optional: true, present: (item: MrItem) => hasValue(item.installBy), render: (item: MrItem) => text(item.installBy, emptyText) },
