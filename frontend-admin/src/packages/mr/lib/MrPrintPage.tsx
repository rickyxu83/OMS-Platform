...
function Header({ order, emptyText, formal }: { order: MrOrder; emptyText: string; formal: boolean }) {
  ...
  const fillDate = hasValue(order.fillDate) ? `填表日期 · ${order.fillDate}` : formal ? '' : `填表日期 · ${emptyText}`
  return <header className="a-header"><div className="a-brand"><img src={`${import.meta.env.BASE_URL}dunyang-mark.png`} alt="" /><div><span>STARK / NINGBO TECHNOLOGY INC.</span><strong>敦阳（宁波）科技有限公司</strong></div></div><div className="a-title"><h1>客户订购申请单（境内单）</h1></div><div className="a-ref"><b>{controlNumber}</b><span>{reference}</span>{fillDate ? <span>{fillDate}</span> : null}</div></header>
}
...
function ItemTable({ items, emptyText, formal }: { items: MrItem[]; emptyText: string; formal: boolean }) {
  const definitions = [
    ...
    { key: 'unitPrice', label: '未税单价', weight: 8, align: 'right', optional: false, present: (item: MrItem) => hasValue(item.unitPrice), render: (item: MrItem) => moneyText(item.unitPrice, emptyText) },
    { key: 'subtotal', label: '未税小计 / 毛利率', title: '未税小计', sub: '/ 毛利率', weight: 9, align: 'right', optional: false, present: (item: MrItem) => hasValue(item.subtotal), render: (item: MrItem) => <span><strong>{moneyText(item.subtotal, emptyText)}</strong><small>{percent(item.marginRate, emptyText)}</small></span> },
    ...
    { key: 'costInclTax', label: '采购成本（含税） / 采购税率', title: '采购成本（含税）', sub: '/ 采购税率', weight: 9, align: 'right', optional: false, present: (item: MrItem) => hasValue(item.costInclTax) || hasValue(item.taxRate), render: (item: MrItem) => <span><strong>{moneyText(item.costInclTax, emptyText)}</strong><small>{hasValue(item.taxRate) ? `${item.taxRate}%` : emptyText}</small></span> },
    { key: 'purchase', label: '采购订单号 / 采购成本来源', title: '采购订单号', sub: '/ 采购成本来源', weight: 9, align: 'left', optional: true, present: (item: MrItem) => hasValue(item.purchaseOrderNo) || hasValue(item.costSource), render: (item: MrItem) => <span><strong>{text(item.purchaseOrderNo, emptyText)}</strong><small>{text(item.costSource, emptyText)}</small></span> },
  ]
  const columns = definitions.filter((column) => !formal || !column.optional || items.some(column.present))
  const totalWeight = columns.reduce((sum, column) => sum + column.weight, 0)
  return <table className="a-items"><thead><tr>{columns.map((column) => <th key={column.key} style={{ width: `${column.weight / totalWeight * 100}%`, textAlign: column.align as 'left' | 'right' | 'center' }}>{column.sub ? <><span className="a-th-title">{column.title}</span><small className="a-th-sub">{column.sub}</small></> : column.label}</th>)}</tr></thead><tbody>{items.map((item, index) => <tr key={item.id || index}>{columns.map((column) => <td key={column.key} data-label={column.label} className={`a-align-${column.align} ${column.key === 'subtotal' ? 'a-strong' : ''}`} title={column.key === 'vendor' ? text(item.vendor, emptyText) : undefined}>{column.render(item, index)}</td>)}</tr>)}</tbody></table>
}
