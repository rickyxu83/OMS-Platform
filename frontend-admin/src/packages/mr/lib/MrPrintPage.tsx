...
function ItemTable({ items, emptyText, formal }: { items: MrItem[]; emptyText: string; formal: boolean }) {
  ...
  return <table className="a-items"><thead><tr>{columns.map((column) => <th key={column.key} style={{ width: `${column.weight / totalWeight * 100}%`, textAlign: 'center', whiteSpace: column.nowrap ? 'nowrap' : undefined }}>{column.sub ? <><span className="a-th-title">{column.title}</span><small className="a-th-sub">{column.sub}</small></> : column.label}</th>)}</tr></thead><tbody>{items.map((item, index) => <tr key={item.id || index}>{columns.map((column) => <td key={column.key} data-label={column.label} className={`a-align-${column.align} ${column.key === 'subtotal' ? 'a-strong' : ''}`} title={column.key === 'vendor' ? text(item.vendor, emptyText) : undefined}>{column.render(item, index)}</td>)}</tr>)}</tbody></table>
}
...
/* ── 现代风：屏幕端视觉升级，打印端黑白兼容 ── */
.mr-print-page{background:#eef1f8;font-family:"Inter","Helvetica Neue",Arial,"PingFang SC","Microsoft YaHei",sans-serif}.mr-document{border:1px solid #e3e7f0;border-top:none;border-radius:16px;box-shadow:0 1px 2px rgba(30,41,59,.04),0 16px 40px rgba(80,70,160,.10)}.mr-document:before{content:"";position:absolute;inset:0 0 auto;height:5px;background:linear-gradient(90deg,#6d5bd0 0%,#8b7cf0 55%,#c4b5fd 100%);z-index:3}.a-header{border-bottom:1px solid #ece9f6}.a-title h1{color:#4b3a9e;letter-spacing:.14em}.a-brand strong,.a-ref b{color:#3c2f7e}.a-orderbar{background:linear-gradient(180deg,#fbfaff,#f5f3fc);border-color:#e9e4f6}.a-orderbar>div{border-color:#efeaf8}.a-section-title{border-bottom:1px solid #e7e3f2}.a-section-title>span{background:linear-gradient(135deg,#6d5bd0,#8b7cf0);color:#fff;border-radius:999px;padding:2px 9px}.a-section-title h2{color:#3c2f7e;font-weight:800}.a-items{border-radius:12px;overflow:hidden;border:1px solid #e6e4f0}.a-items th{background:linear-gradient(135deg,#6d5bd0,#8b7cf0);border-color:#7a6ae0}.a-items th .a-th-sub{color:#e5e0ff}.a-items td{border-color:#ecebf3}.a-items tbody tr:nth-child(even){background:#faf9ff}.a-items tbody tr:hover{background:#f3f1ff}.a-totals{border-color:#e6e2f4;border-radius:12px;overflow:hidden}.a-total{border-color:#efecf8}.a-total:last-child{background:linear-gradient(135deg,#f6f3ff,#ece7ff)}.a-fact-group{background:linear-gradient(180deg,#ffffff,#f6f5fc);border-color:#e6e4f0;border-radius:12px;box-shadow:0 1px 2px rgba(30,41,59,.04)}.a-fact-group-title{color:#3c2f7e}.a-fact-group-title:before{width:7px;height:7px;background:linear-gradient(135deg,#6d5bd0,#8b7cf0)}.a-fact small{color:#7a7f8e}.a-notes-card{background:linear-gradient(180deg,#faf7ff,#f2edff);border-color:#e5ddf5;border-left:4px solid #6d5bd0;border-radius:12px}.a-notes-title{color:#3c2f7e}.a-signatures{gap:10px}.a-signature{border:1px solid #e6e4f0;border-radius:12px;background:linear-gradient(180deg,#ffffff,#faf9fe);box-shadow:0 1px 2px rgba(30,41,59,.04)}.a-signature b{color:#3c2f7e}.a-footer{border-color:#e7e9f0;color:#8b93a1}@media screen{.mr-print-page .a-fact-group{transition:box-shadow .18s ease,transform .18s ease}.mr-print-page .a-fact-group:hover{transform:translateY(-1px);box-shadow:0 6px 18px rgba(80,70,160,.12)}}@media print{.mr-document{border-radius:0;box-shadow:none;border:0}.mr-document:before{display:none}.a-items{border-radius:0;overflow:visible}.a-items th{background:#ddd!important;color:#111!important}.a-items th .a-th-sub{color:#444!important}.a-totals{border-radius:0}.a-total:last-child{background:#f1ecf7!important}.a-section-title>span{background:#ddd!important;color:#111!important}.a-orderbar{background:#fff!important}.a-fact-group,.a-notes-card,.a-signature{border-radius:0;background:#fff!important;box-shadow:none}.a-fact-group-title:before{background:#555!important}.a-notes-card{border-left-color:#777!important}}
`
