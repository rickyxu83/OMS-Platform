...
const FACT_GROUPS = [
  ['客户与合同', ['客户联系人', '客户 P/O', '业务负责人', '项目分类', '合同编号', '付款条件', '付款条件说明', '罚则说明', '填表日期', '采购联系人', '采购联系电话', '采购联系邮箱']],
  ['交易与开票', ['计价模式', '发票类型', '开票方式', '开票内容', '开票/收款时间', '发票收件人', '发票收件电话', '发票收件邮箱']],
  ['交付与验收', ['是否允许分批交付', '验收条件', '验收说明', '装机承担方', '维护承担方', '交付条款', '出货单编号', '收货人', '收货联系电话', '收货邮箱', '交付地点']],
] as const
...
function scheduleEntriesText(entries: unknown, legacyStartMonth: unknown, legacyAmount: unknown, legacyRemaining: unknown, action: string, fallback: string) {
  ...
  if (list.length) {
    const lines = list.map((entry) => {
      ...
      // 无有效总金额（未填或为 0）视为未填写
      if (type === 'once') return total !== null ? `${prefix}一次性${action}，总金额 ${moneyText(total, fallback)}` : ''
      if (total !== null) {
        ...
        return `${prefix}${head}${count}${perText}，总金额 ${moneyText(total, fallback)}`
      }
...
function ItemTable({ items, emptyText, formal }: { items: MrItem[]; emptyText: string; formal: boolean }) {
  ...
  const definitions = [
    ...
    { key: 'partMerged', label: '公司料号 / 原厂规格', weight: 12, align: 'left', optional: true, present: (item: MrItem) => hasValue(item.companyPartNo) || hasValue(item.oemSpec), render: (item: MrItem) => {
      const partNo = hasValue(item.companyPartNo) ? String(item.companyPartNo) : ''
      const spec = hasValue(item.oemSpec) ? String(item.oemSpec) : ''
    ...
    { key: 'warrantyInstall', label: '保固 / 装机', weight: 9, align: 'left', optional: true, present: (item: MrItem) => hasValue(item.warrantyService) || hasValue(item.installBy), render: (item: MrItem) => {
      ...
      if (!warranty && !install) return emptyText
      return <span>{warranty ? <strong>{warranty}</strong> : null}{install ? <small>装机：{install}</small> : null}</span>
    } },
  ...
  return <table className="a-items"><thead><tr>{columns.map((column) => <th key={column.key} style={{ width: `${column.weight / totalWeight * 100}%`, textAlign: column.align as 'left' | 'center' | 'right', whiteSpace: column.nowrap ? 'nowrap' : undefined }}>{column.label}</th>)}</tr></thead><tbody>{items.map((item, index) => <tr key={item.id || index}>{columns.map((column) => {
    ...
    return <td key={column.key} data-label={column.label} className={`a-align-${column.align} ${column.key === 'subtotal' ? 'a-strong' : ''}`}>{column.render(item, index)}</td>
  })}</tr>)}</tbody></table>
}
...
.a-signature img{display:block;margin:3px auto 2px;object-fit:contain}
.a-items td.a-purchase-merged{vertical-align:middle;background:#f0f1f3;font-weight:700;color:#1f2937;text-align:center;border-left:2px solid #111}
.a-items td.a-purchase-merged small{font-weight:400}
@media print{.a-items td.a-purchase-merged{background:#fff!important;border-left:2px solid #111!important}}
@media screen{.mr-print-page .a-section-title>span{font-size:10px}.mr-print-page .a-section-title h2{font-size:15px}.mr-print-page .a-items{font-size:12px}.mr-print-page .a-items th{font-size:12.5px}.mr-print-page .a-items td small{font-size:10.5px}.mr-print-page .a-orderbar small,.mr-print-page .a-total small,.mr-print-page .a-note small{font-size:11px}.mr-print-page .a-orderbar b{font-size:14px}.mr-print-page .a-orderbar span{font-size:11.5px}.mr-print-page .a-total b{font-size:14px}.mr-print-page .a-fact-group-title,.mr-print-page .a-notes-title{font-size:12px}.mr-print-page .a-fact small{font-size:11px}.mr-print-page .a-fact div{font-size:12.5px}.mr-print-page .a-note-row small{font-size:11px}.mr-print-page .a-note-row div{font-size:12px}.mr-print-page .a-signature{min-height:52px;padding:7px 8px}.mr-print-page .a-signature b{font-size:13px}.mr-print-page .a-signature span{font-size:10px;margin-top:3px}.mr-print-page .a-signature strong{font-size:12px;min-height:15px;margin-top:2px}.mr-print-page .a-signature small{font-size:9.5px}.mr-print-page .a-signature img{margin:2px auto 1px}}
@media print{.mr-print-page .a-signature img.a-sig-side{height:34px;max-width:110px}}
/* ── 极简去表格化：无线框、无渐变，细线+留白组织 ── */
.mr-print-page{background:#f7f8fb;font-family:"Inter","Helvetica Neue",Arial,"PingFang SC","Microsoft YaHei",sans-serif}.mr-document{border:none;box-shadow:0 1px 4px rgba(15,23,42,.06);border-radius:12px}.mr-document:before{display:none}.a-header{border-bottom:1px solid #111}.a-title h1{color:#111;letter-spacing:.1em}.a-brand strong{color:#402080}.a-ref b{color:#111}.a-orderbar{background:none;border-top:none;border-bottom:1px solid #e5e7eb}.a-orderbar>div{border-right:none;padding-right:16px}.a-section-title{border-bottom:1px solid #e5e7eb}.a-section-title>span{background:none;color:#6d5bd0;padding:0;letter-spacing:.08em}.a-section-title h2{color:#111;font-weight:700}.a-items{border:none;border-radius:0;overflow:visible}.a-items th{background:none!important;color:#1f2937;border-color:transparent!important;border-bottom:1px solid #111!important;font-weight:700}.a-items th .a-th-sub{color:#667085}.a-items td{border-color:transparent!important;border-bottom:1px solid #f0f2f5!important}.a-items tbody tr:last-child td{border-bottom:none!important}.a-items tbody tr:nth-child(even){background:transparent}.a-items tbody tr:hover{background:#fafbfe}.a-totals{border:1px solid #dfe3ea;border-radius:12px;background:#f5f6fa;overflow:hidden}.a-total{border-right:1px solid #e7eaf0;padding:12px 14px}.a-total:last-child{border-right:none}.a-total small{color:#4d5563}.a-total b{color:#111}.a-total:last-child{background:none}.a-total:last-child b{color:#6d5bd0}.a-total-highlight{background:#ece5f6}.a-total-highlight b{color:#6d5bd0}.a-detail-groups{gap:16px 24px}.a-fact-group{background:none;border:none;border-radius:0;box-shadow:none;padding:2px 0 12px;border-bottom:1px solid #f0f2f5}.a-detail-groups .a-fact-group:nth-last-child(-n+2){border-bottom:none}.a-fact-group-title{color:#111}.a-fact-group-title:before{background:#6d5bd0}.a-fact small{color:#4d5563}.a-notes-card{background:#f8f8fb;border:none;border-left:3px solid #6d5bd0;border-radius:8px}.a-notes-title{color:#111}.a-signature{border:none;border-radius:0;background:none;box-shadow:none;border-left:1px solid #eee;display:flex;align-items:flex-start;gap:10px}.a-signature:first-child{border-left:none}.a-signature .a-sig-text{flex:0 1 auto;min-width:0}.mr-print-page .a-signature img.a-sig-side{display:block;margin:0;padding:0;height:52px;width:auto;max-width:160px;object-fit:contain;object-position:top right;flex-shrink:0}.a-signature b{color:#111}.a-footer{border-top:1px solid #eceef2;color:#667085}.mr-print-toolbar{color:#3f4756}.a-orderbar small,.a-delivery small,.a-note small,.a-orderbar span,.a-delivery span{color:#4d5563}.mr-print-page .a-items td small{color:#4d5563}@media print{.a-items th{background:none!important;color:#111!important;border-bottom:1px solid #111!important;font-weight:700;font-size:9px}.a-items td{border-bottom:1px solid #ddd!important}.a-orderbar{background:none!important}.a-totals{background:#f5f5f5!important;border-radius:0;border:1px solid #111!important}.a-total{border-right-color:#999!important}.a-total-highlight{background:#e8e8e8!important}.a-total:last-child b{color:#111!important}.a-notes-card{background:#f7f7f7!important;border-left-color:#777!important}.a-fact-group-title:before{background:#555!important}.a-section-title>span{color:#111!important}}
`
  ...
  return (
    <div className={`mr-print-page ${embedded ? 'is-embedded' : ''}`}>
      ...
      <article className="mr-document">
        {watermark ? (
          <div className={`a-watermarks${status === 'in_review' ? ' a-watermarks--review' : ''}`}>
            {Array.from({ length: status === 'in_review' ? 3 : 9 }).map((_, index) => <div className="a-watermark" key={index}>{watermark}</div>)}
          </div>
        ) : null}
        ...
        <Section index="01" title={`采购与销售明细 · ${order.items?.length || 0} 个品项`}>
          ...
          <div className="a-totals" style={{ gridTemplateColumns: `repeat(${Math.max(1, totalFacts.length)}, 1fr)`, '--total-columns': Math.max(1, totalFacts.length) } as CSSProperties}>
            {totalFacts.map((fact) => <div className={`a-total${fact.label === '含税总计' ? ' a-total-highlight' : ''}`} key={fact.label}><small>{fact.label}</small><b>{fact.value}</b></div>)}
          </div>
        </Section>
