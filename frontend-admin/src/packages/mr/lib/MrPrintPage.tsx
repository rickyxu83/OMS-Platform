  ...
  return (
    <div className={`mr-print-page ${embedded ? 'is-embedded' : ''}`}>
      ...
      <article className="mr-document">
        {watermark ? (
          <div className="a-watermarks">
            {Array.from({ length: 9 }).map((_, index) => <div className="a-watermark" key={index}>{watermark}</div>)}
          </div>
        ) : null}
