...
function ItemTable({ items, emptyText, formal }: { items: MrItem[]; emptyText: string; formal: boolean }) {
  const definitions = [
    ...
    { key: 'costExcludingTax', label: '采购成本（未税）', weight: 9, align: 'right', optional: false, present: (item: MrItem) => hasValue(item.costExcludingTax), render: (item: MrItem) => moneyText(item.costExcludingTax, emptyText) },
    ...
    { key: 'purchase', label: '采购单号', weight: 7, align: 'left', optional: true, present: (item: MrItem) => hasValue(item.purchaseOrderNo), render: (item: MrItem) => text(item.purchaseOrderNo, emptyText) },
    // 出货单号列固定保留：系统已填则印出，未填留白供出货时手写
  ...
  // 连续相同采购单号合并为一个单元格（rowSpan），避免整列重复同一个单号
...
.a-items td.a-purchase-merged{vertical-align:middle;background:#f7f5fb;font-weight:700;color:#3e2d59;text-align:center;border-left:2px solid #73529b}
