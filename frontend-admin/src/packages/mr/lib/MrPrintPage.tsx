...
function Header({ order, emptyText, formal }: { order: MrOrder; emptyText: string; formal: boolean }) {
  ...
  return <header className="a-header"><div className="a-brand"><img src={`${import.meta.env.BASE_URL}dunyang-mark.png`} alt="" /><div><span>STARK (NINGBO) TECHNOLOGY INC.</span><strong>敦阳（宁波）科技有限公司</strong></div></div><div className="a-title"><h1>客户订购申请单（境内单）</h1><p>Customer Order Request (Domestic)</p></div><div className="a-ref"><span>单据编号</span><b>{controlNumber}</b></div></header>
}
function Fact({ label, value }: { label: string; value: ReactNode }) { return <div className="a-fact"><small>{label}</small><div>{value}</div></div> }
function Section({ index, title, en, children }: { index: string; title: string; en?: string; children: ReactNode }) { return <section className="a-section"><div className="a-section-title"><span>{index}</span><h2>{title}{en ? <small className="a-section-en">{en}</small> : null}</h2></div>{children}</section> }
  ...
  return (
    <div className={`mr-print-page ${embedded ? 'is-embedded' : ''}`}>
      ...
      <article className="mr-document">
        ...
        <Section index="01" title={`采购与销售明细 · ${order.items?.length || 0} 个品项`} en="Purchase & Sales Details">
        ...
        <Section index="02" title={`订购与交付资料 · 共 ${facts.length + noteFacts.length} 项`} en="Order & Delivery Information">
        ...
        <Section index="03" title="电子签核记录" en="Approval Records"><Signatures order={order} formal={formal} /></Section>
