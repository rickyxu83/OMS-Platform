      ...
      <article className="mr-document">
        {watermark ? (
          <div className={`a-watermarks${status === 'in_review' ? ' a-watermarks--review' : ''}`}>
            {Array.from({ length: status === 'in_review' ? 1 : 9 }).map((_, index) => <div className="a-watermark" key={index}>{watermark}</div>)}
          </div>
        ) : null}
