...
6. **分支先行，无例外（2026-09-14 起，基于值班津贴 demo 在 main 工作区直接改动的教训）**：凡是要写入/修改仓库内文件的任务，第一步必须先 `git checkout -b feat|fix|hotfix/<描述>`（或确认当前已在短命分支上），然后才动手写代码。禁止自行判断"这个改动不够格走流程"——本地 demo、预览页、原型、临时脚本同样适用；"只是本地看看、不部署、不落库"不是跳过分支的理由。
